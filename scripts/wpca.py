...
# filter since and remove testing votes (sitting 999)
vote_events = pd.read_csv(localpath + "data/vote_events.csv")
selected_vote_events = vote_events[(vote_events['date'] >= since) & (vote_events['sitting'] != 999)]
Xsource = Xsource[Xsource['vote_event_id'].isin(selected_vote_events['vote_event_id'])]
Xsource = Xsource.merge(selected_vote_events.loc[:, ['vote_event_id', 'date']], on='vote_event_id', how='left')
Xsource['voter_id'] = Xsource['voter_id'].astype('Int64')
...
out.to_csv(localpath + "data/wpca2023.csv", index=False)



# PROJECTIONS, TIME INTERVALS
# time intervals = 1 year
lo_limitT = 0.1
# Convert the 'date' column to datetime type
vote_events = pd.read_csv(localpath + "data/vote_events.csv")
vote_events['date'] = pd.to_datetime(vote_events['date'])

# selected only vote events since since and without testing votes (sitting 999)
vote_events = vote_events[(vote_events['date'] >= since) & (vote_events['sitting'] != 999)]
...
# Iterate over each year period
current_date = first_date
while current_date <= last_date:
  # Calculate the start and end dates of the current half year period
  period_start = pd.Timestamp(current_date.year, 1, 1)
  period_end = pd.Timestamp(current_date.year, 12, 31, 23, 59, 59)
...
out_columns = ['voter_id', 'dim1', 'dim2', 'dim3', 'jméno', 'kandidátka', 'klub']
...
for i, projection in enumerate(projections):
  ...
  p = p.merge(mps.loc[:, ['mp_id', 'name', 'party']], left_on='voter_id', right_on='mp_id', how='left')
  p = p[p.duplicated(subset=['voter_id'], keep='last') == False]
  p['datum'] = i

  out = pd.concat([out, p], axis=0)

out.to_csv(localpath + "data/wpca.time.v1.csv", index=False)
